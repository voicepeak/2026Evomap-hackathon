    ...
    def _setup_motors(self) -> None:
        for jc in self._all_joints:
            ...
            if vendor == "damiao":
                ...
                # RobStride motors can be latched in a fault/no-torque state after a
                # stall or a parameter write (position stops tracking, iq stays 0).
                # Clear it once at connect so every example starts from a live motor.
                try:
                    mot.clear_error()
                except Exception:
                    pass
