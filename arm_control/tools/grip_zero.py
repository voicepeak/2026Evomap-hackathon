...
流程（按 --dir 给的方向）：
    ...
    → --span：再往反方向走到另一端，打印整程和建议的 lo_deg/hi_deg
...
用法（**先停掉采集服务**；PCAN 被服务独占时连不上）：
    ...
    #    爪片在合 → 方向对了；在开 → 换 --dir +1
    # 2) 把"合到位"设成零点，并量出整程：
    capture_platform/.venv/bin/python arm_control/tools/grip_zero.py --dir -1 --set-zero --span
    # 3) 按打印的建议改 capture_platform/configs/rebot_b601_rs.json 的
    #    gripper.lo_deg / hi_deg，再重启服务
    ...
    try:
        ...
        if args.span and hit:
            ...
            print(f"\n📏 反向到另一端：{math.degrees(s2):+.1f}° → {math.degrees(e2):+.1f}°"
                  f"（{'到端' if hit2 else '未确认到端'}）", flush=True)
            if args.set_zero:
                low = min(math.degrees(s2), math.degrees(e2), 0.0)
                high = max(math.degrees(s2), math.degrees(e2), 0.0)
                print("   → capture_platform/configs/rebot_b601_rs.json 建议改成：", flush=True)
                print(f'     "lo_deg": {max(0.0, low + 3.0):.0f},  "hi_deg": {high - 5.0:.0f}'
                      f"   （留 3°/5° 余量；改完重启服务）", flush=True)
            else:
                print("   （这次没设零点，范围是按旧零点算的；要得到可直接填配置的范围，"
                      "加 --set-zero 再跑一次）", flush=True)
    finally:
